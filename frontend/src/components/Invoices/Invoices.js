...
import { selectIsLoading, get_Invoice_by_User, delete_Invoice } from '../../redux/features/invoice/invoice';
...
const Invoices = () => {
  ...
  const handleMouseLeave = () => {
    ...
    useEffect(() => {
      ...
      dispatch(get_Invoice_by_User({ search: user?.result?._id}))
    },[location])
              ...
              .map((invoice) => {
                return (
                  <TableRow  role="checkbox" tabIndex={-1} key={invoice._id}>
                    ...
                    <TableCell align='center' onClick={() => deleteInvoice(invoice._id)} sx={{backgroundColor:"", width:"50px", cursor:"pointer"}}><DeleteIcon/></TableCell>
