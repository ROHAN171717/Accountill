...
import { create_Client, get_Client_by_User } from '../../redux/features/client/client';
...
const Invoice = () => {
  ...
  const handleClickClose = () => {
    ...
    React.useEffect(() => {
      // console.log("error from client...by search...")
  ...
  // console.log(invoiceData)
    ...
    const handleChange =(index, e) => {
      ...
      console.log(values[index][e.target.name])
      values[index][`${e.target.name}`] = e.target.value;
      console.log("new="+values[index][e.target.name])
...
const addClient = () => {
  console.log("edit...")
  dispatch(create_Client({...clientData, userId: user?.result?._id}))
  // dispatch(get_Client_by_User({ search: user?.result?._id }))
}
  ...
  const handleSubmit = async (e) => {
    ...
    if(invoice){
      ...
      dispatch(update_Invoice({id, formData}))
      navigate(`/invoice/${invoice._id}`)
    }else{
      dispatch(create_Invoice({
        ...invoiceData,
        subTotal: subTotal, 
        total: total, 
        vat: vat, 
